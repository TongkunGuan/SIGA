...
class Model(nn.Module):
    ...
    def __init__(self, opt):
        ...
        if opt.Transformer == "char-str":
            print("USE CHAR-STR")
            self.mgp_str= create_char_str(batch_max_length=opt.batch_max_length+1, num_tokens=opt.num_class, model=opt.TransformerModel)
    ...
    def forward(self, input, mask, text, len_target, iteration, is_eval=False):
