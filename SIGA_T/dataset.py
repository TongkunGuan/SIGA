...
class Batch_Balanced_Dataset(object):
    ...
    def get_batch(self):
        ...
        for i, data_loader_iter in enumerate(self.dataloader_iter_list):
            try:
                image, mask, text, _ = data_loader_iter.next()
                balanced_batch_images.append(image)
                balanced_batch_masks.append(mask)
                balanced_batch_texts += text
            except StopIteration:
                self.dataloader_iter_list[i] = iter(self.data_loader_list[i])
                image, mask, text, _ = self.dataloader_iter_list[i].next()
                balanced_batch_images.append(image)
                balanced_batch_masks.append(mask)
                balanced_batch_texts += text
            except ValueError:
        ...
        return balanced_batch_images, balanced_batch_masks, balanced_batch_texts
    ...
    def __call__(self, batch):
        ...
        if self.keep_ratio_with_pad:  # same concept with 'Rosetta' paper
            ...
            for image in images:
                ...
                mask = Image.fromarray(clusterpixels(image.convert("L"), 2))
                resized_mask = mask.resize((resized_w, self.imgH), Image.BICUBIC)
                ...
                aug_masks = aug_transform(resized_mask)
                pad_masks = transform(aug_masks)
                resized_masks.append(pad_masks)
                
            image_tensors = torch.cat([t.unsqueeze(0) for t in resized_images], 0)
            mask_tensors = torch.cat([t.unsqueeze(0) for t in resized_masks], 0)
            ...
            for image in images:
                ...
                mask = Image.fromarray(clusterpixels(image.convert("L"), 2))
                image, mask = aug_transform(image, mask)
                image_tensors.append(image)
                mask_tensors.append(mask)
            image_tensors = torch.cat([t.unsqueeze(0) for t in image_tensors], 0)
            mask_tensors = torch.cat([t.unsqueeze(0) for t in mask_tensors], 0)

        return image_tensors, mask_tensors, labels, img_paths
