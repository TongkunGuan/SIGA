    ...
    def encode(self, text):
        ...
        for i, t in enumerate(text):
            ...
            try:
                ...
                batch_text[i][:self.batch_max_length] = torch.LongTensor(txt)[:self.batch_max_length]
        return batch_text.to(device)
    
    def char_encode(self, text):
        """ convert text-label into text-index.
        """
        batch_len = torch.LongTensor(len(text)).fill_(self.dict[self.GO])
        batch_text = torch.LongTensor(len(text), self.batch_max_length).fill_(self.dict[self.GO])
        for i, t in enumerate(text):
            length = len(t) 
            batch_len[i] = torch.LongTensor([length + 1])  # batch_text[:, 0] = [GO] token
...
def get_args(is_train=True):
    ...
    parser.add_argument('--showInterval', type=int, default=500, help='Interval between each validation')
