...
class Shrink:
    ...
    def __call__(self, img, mask, mag=-1, prob=1.):
        ...
        mask = self.tps.warpImage(mask.astype(np.uint8))
        mask = Image.fromarray(mask)

        if np.random.uniform(0, 1) < 0.5:
            img = self.translateXAbs(img, val=x)
            mask = self.translateXAbs(mask, val=x)
        else:
...
class Rotate:
    ...
    def __call__(self, img, mask, iscurve=False, mag=-1, prob=1.):
        if np.random.uniform(0,1) > prob:
            return img, mask
        ...
        if H!=self.side or W!=self.side:
            ...
            mask = mask.resize((self.side, self.side), Image.BICUBIC)
        ...
        angle = np.random.uniform(rotate_angle-10, rotate_angle)
        if np.random.uniform(0, 1) < 0.5:
            angle = -angle
        ...
        mask = mask.rotate(angle=angle, resample=Image.BICUBIC, expand=expand)
