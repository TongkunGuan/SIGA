...
from .segmentor import SegHead
from .predicition import Attention, Attention2D, Attention_Fusion, GatedBimodal
from .sequential_model import BidirectionalLSTM
from loss import STR_Loss
...
class CHARSTR(VisionTransformer):
    ...
    def __init__(self, batch_max_length, *args, **kwargs):
        ...
        self.segmentation = SegHead(in_channels=self.embed_dim, mla_channels=128, mlahead_channels=128, num_classes=self.batch_max_length)
    ...
    def reset_classifier(self, num_classes):
        ...
        self.Prediction_our = Attention(256, 256, num_classes)
        self.Prediction_TRBA = Attention(256, 256, num_classes)
        self.FC_Prediction_1 = Attention2D(256, 256, num_classes)
        self.FC_Prediction_2 = Attention2D(256, 256, num_classes)
    ...
    def forward(self, x, mask, text, length, iteration, is_eval=False):
        sequential_features, glyph_features, F_score, char_feature_attn = self.forward_features(x)
        ...
        Ours_pred, seq_attention_map, Ours_hiddens = self.Prediction_our(contextual_feature.contiguous(),
                                            text, is_train=not is_eval, batch_max_length=self.batch_max_length)
        contextual_feature = self.SequenceEncoding(visual_feature)
        TRBA_pred, TRBA_attn, TRBA_hiddens = self.Prediction_TRBA(contextual_feature.contiguous(),
                                            text, is_train=not is_eval, batch_max_length=self.batch_max_length)

        Attentive_Sequence_1 = F.dropout(glyph_features[0], p=0.3, training=not is_eval)
        pre2D_iter1, _ = self.FC_Prediction_1(Attentive_Sequence_1.contiguous(), text, is_train=not is_eval,
                                              batch_max_length=self.batch_max_length)
        Attentive_Sequence_2 = F.dropout(glyph_features[1], p=0.3, training=not is_eval)
        pre2D_iter2, _ = self.FC_Prediction_2(Attentive_Sequence_2.contiguous(), text, is_train=not is_eval,
                                              batch_max_length=self.batch_max_length)
        ...
        Complementary_features = F.dropout(Complementary_features, p=0.3, training=not is_eval)
        Share_pred, output_hiddens = self.generator(Complementary_features.contiguous(),
                                                    text, is_train=not is_eval, batch_max_length=self.batch_max_length)
        if is_eval:
            return Ours_pred, TRBA_pred, (pre2D_iter1, pre2D_iter2), (Iter_pred1, Iter_pred2), Share_pred
        else:
            loss, Single_char_mask, fore_mask = self.loss(mask, F_score, seq_attention_map, char_feature_attn, length, iteration, alpha=0.05)
            map = (seq_attention_map, char_feature_attn, Single_char_mask, fore_mask)
            return Ours_pred, TRBA_pred, (pre2D_iter1, pre2D_iter2), (Iter_pred1, Iter_pred2), Share_pred, loss, map
