        ...
        if self.mode == 'train':
            with self.mask_env.begin(write=False) as txn:
                try:
                    ...
                    imgbuf = txn.get(mask_key)
                    mask = np.frombuffer(imgbuf, dtype=np.uint8).reshape(image.height, image.width)
                except:
                    print(f"Corrupted image for {index}")
                    mask = np.zeros((self.opt.imgW, self.opt.imgH))
        else:
            mask = None
    ...
    def __call__(self, batch):
        ...
        if self.mode == "train":
            ...
            for image, gray in zip(images, masks):
                ...
                mask_tensors.append(self.mask_transfrom(mask.convert('I')))
