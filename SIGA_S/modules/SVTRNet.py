...
def drop_path(x, drop_prob=0., training=False):
    ...
    output = x.div(keep_prob) * random_tensor.to(device)
    return output
...
class DropPath(nn.Module):
    ...
    def forward(self, x):
        if self.drop_prob == 0. or not self.training:
            return x
        keep_prob = torch.as_tensor(1 - self.drop_prob)
        shape = (x.shape[0],) + (1,) * (x.ndim - 1)
        random_tensor = keep_prob + torch.rand(shape, dtype=x.dtype)
        random_tensor = torch.floor(random_tensor)  # binarize
        output = x.div(keep_prob) * random_tensor.to(device)
    ...
    def __init__(self,
        ...
        if sub_num == 2:
            self.proj0 = nn.Sequential(
                nn.Conv2d(in_channels=in_channels, out_channels=embed_dim // 2, kernel_size=3, stride=2, padding=1, groups=1, bias=True),
                nn.BatchNorm2d(embed_dim // 2),
                nn.GELU()
            )
            self.proj1 = nn.Sequential(
                nn.Conv2d(in_channels=embed_dim // 2, out_channels=embed_dim, kernel_size=3, stride=2, padding=1, groups=1, bias=True),
...
class SVTRNet(nn.Module):
    def __init__(
        ...
        if not prenorm:
            self.norm = nn.LayerNorm(embed_dim[-1], eps=epsilon)
    ...
    def forward_features(self, x):
        FPN = []
        x0, x = self.patch_embed(x)
        FPN.append(x0.contiguous()) # 24, 80, 96
        x = x + self.pos_embed
        x = self.pos_drop(x)
        for blk in self.blocks1:
            x = blk(x)
        FPN.append(x.permute(0, 2, 1).reshape([-1, self.embed_dim[0], self.HW[0], self.HW[1]]).contiguous())  # 12, 40, 192
        ...
        for blk in self.blocks2:
            x = blk(x)
        FPN.append(x.permute(0, 2, 1).reshape([-1, self.embed_dim[1], self.HW[0] // 2, self.HW[1]]).contiguous())  # 6, 40, 256
        if self.patch_merging is not None:
            x = self.sub_sample2(
                x.permute(0, 2, 1).reshape(
...
def drop_path(x, drop_prob=0., training=False):
    ...
    random_tensor = keep_prob + torch.rand(shape, dtype=x.dtype)
    random_tensor = torch.floor(random_tensor)  # binarize
    output = x.div(keep_prob) * random_tensor.cuda()
    return output
...
class SVTRNet(nn.Module):
    ...
    def forward_features(self, x):
        FPN = []
        x0, x = self.patch_embed(x)
        FPN.append(x0) # 24, 80, 96
        x = x + self.pos_embed
        x = self.pos_drop(x)
        for blk in self.blocks1:
            x = blk(x)
        FPN.append(x.permute(0, 2, 1).contiguous().reshape([-1, self.embed_dim[0], self.HW[0], self.HW[1]]))  # 12, 40, 192
        if self.patch_merging is not None:
            x = self.sub_sample1(
                x.permute(0, 2, 1).reshape(
                    [-1, self.embed_dim[0], self.HW[0], self.HW[1]]))
        for blk in self.blocks2:
            x = blk(x)
        FPN.append(x.permute(0, 2, 1).contiguous().reshape([-1, self.embed_dim[1], self.HW[0] // 2, self.HW[1]]))  # 6, 40, 256
        if self.patch_merging is not None:
            x = self.sub_sample2(
                x.permute(0, 2, 1).reshape(
                    [-1, self.embed_dim[1], self.HW[0] // 2, self.HW[1]]))
