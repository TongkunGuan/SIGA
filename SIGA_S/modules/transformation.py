    ...
    def forward(self, batch_I, Mask):
        ...
        if Mask is not None:
            ...
            if torch.__version__ > "1.2.0":
                ...
                batch_M_r = F.grid_sample(Mask, build_P_prime_reshape, padding_mode='border', align_corners=True)
            else:
                batch_I_r = F.grid_sample(batch_I, build_P_prime_reshape, padding_mode="border")
                batch_M_r = F.grid_sample(Mask, build_P_prime_reshape, padding_mode='border')
        else:
            if torch.__version__ > "1.2.0":
                batch_I_r = F.grid_sample(batch_I, build_P_prime_reshape, padding_mode="border", align_corners=True, )
            else:
...
class GridGenerator(nn.Module):
    ...
    def __init__(self, F, I_r_size):
        ...
        if num_gpu > 1:
            ...
            self.P_hat = (
                torch.tensor(self._build_P_hat(self.F, self.C, self.P)).float().to(device)
            )  # n x F+3
