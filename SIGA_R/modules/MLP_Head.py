...
class MLP(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_blocks):
        ...
        self.conv3 = nn.Sequential(
            nn.Conv2d(hidden_size, hidden_size, kernel_size=2, stride=(2, 1), padding=(0, 1), bias=False),
            nn.BatchNorm2d(hidden_size),
            nn.ReLU(inplace=True))
        """
        # self.conv1 = nn.Sequential(nn.Conv2d(input_size, hidden_size, kernel_size=1, stride=1, padding=0, bias=False),
        #                            nn.BatchNorm2d(hidden_size),
        #                            nn.ReLU(inplace=True))
...
class Multi_MLP(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, input_H=16, input_W=50, num_blocks=1, iterable=1):
        ...
        self.conv3 = nn.Sequential(
            ...
            nn.BatchNorm2d(hidden_size),
            nn.ReLU(inplace=True))
        np.floor((input_W+1)/2).astype(np.int), (8,2)
        np.floor((input_W+1)/1).astype(np.int), (8,1)
        """
        if iterable == 1:
